...
import { Args, ClientFactory, ProviderType, WalletClient, EOperationStatus, u64ToBytes, u8toByte, ON_MASSA_EVENT_DATA, ON_MASSA_EVENT_ERROR, EventPoller, toMAS, utils, } from '@massalabs/massa-web3';
import { readFileSync } from 'fs';
import path from 'path';
import { fileURLToPath } from 'url';
import { calculateCoinsSent, calculateBytecodeSize, calculateMaxCoins, } from './utils';
...
async function checkBalance(web3Client, account, requiredBalance) {
    ...
    const candidateBalance = balance?.candidate.toString() || '0';
    const finalBalance = balance?.final.toString() || '0';
    console.log(`Wallet Address: ${account.address} has balance (candidate, final) = (${toMAS(candidateBalance)}, ${toMAS(finalBalance)})`);
    if (!balance?.final || balance.final < requiredBalance) {
        throw new Error(`Insufficient MAS balance. Required: ${toMAS(requiredBalance.toString())}`);
    }
}
/**
 ...
 */
async function awaitOperationFinalization(web3Client, operationId) {
...
const pollAsyncEvents = async (web3Client, opId) => {
    ...
    return new Promise((resolve, reject) => {
        eventPoller.on(ON_MASSA_EVENT_DATA, (events) => {
            ...
            let errorEvents = events.filter((e) => e.data.includes(MASSA_EXEC_ERROR));
 ...
 */
async function deploySC(publicApi, account, contracts, chainId, fee = 0n, maxGas = 1000000n, wait = false, maxCoins = 0n) {
    ...
    contracts.forEach((contract, i) => {
        ...
        if (contract.args) {
            datastore.set(new Uint8Array(new Args()
                .addU64(BigInt(i + 1))
                .addUint8Array(u8toByte(0))
                .serialize()), new Uint8Array(contract.args.serialize()));
        }
        if (contract.coins > 0) {
            datastore.set(new Uint8Array(new Args()
                .addU64(BigInt(i + 1))
                .addUint8Array(u8toByte(1))
                .serialize()), u64ToBytes(BigInt(contract.coins)));
        }
    });
    ...
    const { isError, eventPoller, events } = await utils.time.withTimeoutRejection(pollAsyncEvents(client, opId), 60000);
    // stop polling
    eventPoller.stopPolling();
    // if errors, don't await finalization
    if (isError) {
        throw new Error(`Massa Deployment Error: ${JSON.stringify(events, null, 4)}`);
    }
    // await finalization
