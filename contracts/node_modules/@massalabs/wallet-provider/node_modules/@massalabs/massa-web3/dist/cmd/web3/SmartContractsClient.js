...
class SmartContractsClient extends BaseClient_1.BaseClient {
    ...
    async callSmartContract(callData, executor) {
        ...
        if (callData.maxGas > web3_utils_1.MAX_GAS_CALL) {
            throw new Error(`The gas submitted ${callData.maxGas.toString()} exceeds the max. allowed block gas of ${web3_utils_1.MAX_GAS_CALL.toString()}`);
        }
        callData.coins = callData.coins || BigInt(0);
        // check that the sender has enough balance to pay for coins
        const senderBalance = await this.walletClient.getAccountBalance(sender.address());
        if (senderBalance.final < callData.coins) {
            throw new Error(`The sender ${sender.address()} does not have enough balance to pay for the coins`);
        }
    ...
    async readSmartContract(readData) {
        // check the max. allowed gas
        if (readData.maxGas > web3_utils_1.MAX_GAS_CALL) {
            throw new Error(`The gas submitted ${readData.maxGas.toString()} exceeds the max. allowed block gas of ${web3_utils_1.MAX_GAS_CALL.toString()}`);
        }
        if (readData.parameter instanceof web3_utils_1.Args)
    ...
    async getContractBalance(address) {
        ...
        return {
            candidate: (0, web3_utils_1.fromMAS)(addressInfo.candidate_balance),
            final: (0, web3_utils_1.fromMAS)(addressInfo.final_balance),
        };
    }
    /**
    ...
    async getOperationStatus(opId) {
        ...
        const { is_operation_final, op_exec_status, in_pool, in_blocks } = operationData[0];
        if (is_operation_final === null && op_exec_status === null)
            return EOperationStatus_1.EOperationStatus.UNEXECUTED_OR_EXPIRED;
        if (in_pool)
            return EOperationStatus_1.EOperationStatus.AWAITING_INCLUSION;
        if (is_operation_final) {
            if (op_exec_status)
                return EOperationStatus_1.EOperationStatus.FINAL_SUCCESS;
            // We explicitly check for false here because null means that the operation was not executed
            if (op_exec_status === false)
                return EOperationStatus_1.EOperationStatus.FINAL_ERROR;
        }
        else {
            if (op_exec_status)
                return EOperationStatus_1.EOperationStatus.SPECULATIVE_SUCCESS;
            if (op_exec_status === false)
                return EOperationStatus_1.EOperationStatus.SPECULATIVE_ERROR;
        }
     ...
     * @returns A promise that resolves to the status of the operation.
     */
    async awaitRequiredOperationStatus(opId, requiredStatus, timeout) {
        return await this.awaitOperationStatusHelper(opId, timeout, (currentStatus) => currentStatus === requiredStatus);
    }
    /**
     ...
     * @returns A promise that resolves to the status of the operation.
     */
    async awaitMultipleRequiredOperationStatus(opId, requiredStatuses, timeout) {
        return await this.awaitOperationStatusHelper(opId, timeout, (currentStatus) => requiredStatuses.includes(currentStatus));
    }
    /**
     ...
     *
     */
    async awaitOperationStatusHelper(opId, timeout = WAIT_STATUS_TIMEOUT, statusCheck) {
        const startTime = Date.now();
        while (Date.now() - startTime < timeout) {
            let currentStatus = EOperationStatus_1.EOperationStatus.NOT_FOUND;
            try {
        ...
        throw new Error(`Failed to retrieve status of operation id: ${opId}: Timeout reached.`);
    }
}
