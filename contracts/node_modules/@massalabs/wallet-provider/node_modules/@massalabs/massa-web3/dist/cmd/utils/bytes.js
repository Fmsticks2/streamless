...
function getBytesSecretKey(secretKey) {
    ...
    if (!(prefix == SECRET_KEY_PREFIX)) {
        throw new Error(`Invalid secret key prefix: "${prefix}". The secret key should start with "${SECRET_KEY_PREFIX}". Please verify your secret key and try again.`);
    }
    const secretKeyBase58Decoded = (0, Xbqcrypto_1.base58Decode)(secretKey.slice(SECRET_KEY_PREFIX_LENGTH));
...
function getBytesPublicKey(publicKey) {
    ...
    if (!(prefix == PUBLIC_KEY_PREFIX)) {
        throw new Error(`Invalid public key prefix: ${prefix} should be ${PUBLIC_KEY_PREFIX}`);
    }
    const publicKeyBase58Decoded = (0, Xbqcrypto_1.base58Decode)(publicKey.slice(PUBLIC_KEY_PREFIX_LENGTH));
