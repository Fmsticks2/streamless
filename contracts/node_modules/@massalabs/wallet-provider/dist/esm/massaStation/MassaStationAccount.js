...
export class MassaStationAccount {
    ...
    async balance() {
        ...
        try {
            signOpResponse = await getRequest(`${MASSA_STATION_URL}massa/addresses?attributes=balance&addresses=${this._address}`);
        }
        catch (ex) {
    ...
    async sign(data) {
        ...
        try {
            signOpResponse = await postRequest(`${MASSA_STATION_ACCOUNTS_URL}/${this._name}/signMessage`, signData);
        }
        catch (ex) {
        ...
        const signature = base58Encode(Buffer.from(signOpResponse.result.signature, 'base64'));
        return {
