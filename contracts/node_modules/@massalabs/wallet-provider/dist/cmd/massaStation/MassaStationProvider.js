...
class MassaStationProvider {
    ...
    constructor(infos) {
        ...
        this.massaStationEventsListener = new events_1.default();
    }
    /**
    ...
    async getNetwork() {
        try {
            const nodesResponse = await (0, RequestHandler_1.getRequest)(`${exports.MASSA_STATION_URL}massa/node`);
            if (nodesResponse.isError || nodesResponse.error) {
                throw nodesResponse.error.message;
            }
    ...
    listenNetworkChanges(callback) {
        this.massaStationEventsListener.on(MASSA_STATION_NETWORK_CHANGED, (evt) => callback(evt));
        // check periodically if network changed
        const intervalId = setInterval(async () => {
            const currentNetwork = this.currentNetwork?.name;
            const network = await this.getNetwork();
            if (currentNetwork !== network) {
                this.massaStationEventsListener.emit(MASSA_STATION_NETWORK_CHANGED, network);
            }
        }, 500);
        return {
            unsubscribe: () => {
                clearInterval(intervalId);
                this.massaStationEventsListener.removeListener(MASSA_STATION_NETWORK_CHANGED, 
    ...
    async connect() {
        throw new Error('connect functionality is not yet implemented for the current provider.');
    }
    async disconnect() {
        throw new Error('disconnect functionality is not yet implemented for the current provider.');
    }
    connected() {
        throw new Error('connected functionality is not yet implemented for the current provider.');
    }
    enabled() {
        throw new Error('enabled functionality is not yet implemented for the current provider.');
    }
}
